...
{ 	
   // Turn display off
		ssd1306_sendCommand(OLED_DISPLAYOFF);

		ssd1306_sendCommand(OLED_SETDISPLAYCLOCKDIV);
		ssd1306_sendCommand(0x80);

		ssd1306_sendCommand(OLED_SETMULTIPLEX);
		//sendCommand(0x1F);//128x32
		ssd1306_sendCommand(0x3F);//128x64
		
		ssd1306_sendCommand(OLED_SETDISPLAYOFFSET);
		ssd1306_sendCommand(0x00);
		
		ssd1306_sendCommand(OLED_SETSTARTLINE | 0x00);//0
		
		// We use internal charge pump
		ssd1306_sendCommand(OLED_CHARGEPUMP);
		ssd1306_sendCommand(0x14);
		
		// Horizontal memory mode
		ssd1306_sendCommand(OLED_MEMORYMODE);
		ssd1306_sendCommand(0x00);
		
		ssd1306_sendCommand(OLED_SEGREMAP | 0x1);

		ssd1306_sendCommand(OLED_COMSCANDEC);

		ssd1306_sendCommand(OLED_SETCOMPINS);
		//sendCommand(0x02);//128x32
		ssd1306_sendCommand(0x12);//128x64
		
		// Max contrast
		ssd1306_sendCommand(OLED_SETCONTRAST);
		//sendCommand(0x0F);//0xCF
		ssd1306_sendCommand(0xCF);//0xCF

		ssd1306_sendCommand(OLED_SETPRECHARGE);
		ssd1306_sendCommand(0xF1);

		ssd1306_sendCommand(OLED_SETVCOMDETECT);
		//sendCommand(0x10);//0x40
		ssd1306_sendCommand(0x40);//0x40

		ssd1306_sendCommand(OLED_DISPLAYALLON_RESUME);

		// Non-inverted display
		ssd1306_sendCommand(OLED_NORMALDISPLAY);

		// Turn display back on
		ssd1306_sendCommand(OLED_DISPLAYON);  
}
/************************************************************
*************************************************************/
