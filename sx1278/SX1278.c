		...
		uint8_t length) 
{
	...
	if (length <= 1) {
		...
		SX1278_ReadBurst( addr, rxBuf, length);
		}
		SX1278_hw_SetNSS(1);
...
void SX1278_SPIBurstWrite(uint8_t addr, uint8_t *txBuf,
	...
	if (length <= 1) {
	   ...
	   SX1278_WriteBurst(addr, txBuf, length);
		SX1278_hw_SetNSS(1);
	}
}
/************************************************************
...
int SX1278_LoRaEntryRx(SX1278_t *module, uint8_t length, uint32_t timeout) {
	...
	SX1278_SPIWrite(REG_LR_DIOMAPPING1, 0x00);//DIO=00,DIO1=00,DIO2=00, DIO3=00
	SX1278_SPIWrite(LR_RegIrqFlagsMask, 0xBF);//Open RxDone interrupt 
